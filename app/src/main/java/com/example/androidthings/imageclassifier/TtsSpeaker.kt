...
class TtsSpeaker {
    ...
    fun speakResults(tts: TextToSpeech, results: Collection<Recognition>) {
        if (results.isEmpty()) {
    ...
    private class SupermanUtterance : Utterance {
        override fun speak(tts: TextToSpeech) {
            tts.setPitch(1.8f)
    ...
    private class LooksLikeMeUtterance : Utterance {
        override fun speak(tts: TextToSpeech) {
            tts.setPitch(1.3f)
    ...
    private class LensCapOnUtterance : Utterance {
        override fun speak(tts: TextToSpeech) {
            tts.setPitch(0.7f)
            tts.setSpeechRate(1.6f)
    ...
    companion object {
        ...
        private val HUMOR_THRESHOLD = 0.3f
        ...
        init {
            ...
            JOKES.add(SupermanUtterance())
            JOKES.add(LooksLikeMeUtterance())
            JOKES.add(LensCapOnUtterance())
        }
